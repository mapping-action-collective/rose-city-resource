...
pool.on("error", async (error, client) => {
  if (isDevEnvironment) {
    console.error(
      `Database pool error: ${error}; Connection string: ${keys.DATABASE_URL}`
    );
  }
});
...
app.use(function (req, res, next) {
  ...
  const cacheControlHeaderValue = isProdEnvironment
    ? `public, max-age=${
        (expireAfterMinutes / 2) * 60
      }, stale-while-revalidate=${(expireAfterMinutes / 2) * 60}`
    : `no-cache`;
...
const testDatabaseQuery = () => {
  pool.query(`select last_update from production_meta`, (err, res) => {
    if (err) {
      ...
      if (
        keys.DATABASE_URL === undefined ||
        keys.DATABASE_URL === null ||
        keys.DATABASE_URL === ""
      ) {
        console.error(
          "Please check that the DATABASE_URL environment variable is correct. See comments in nodeKeys.js for further information."
        );
      }
    }
  });
...
if (process.env.NODE_ENV === "production") {
  ...
  const frontEndPath = path.join(__dirname, "/../frontend/dist/apps/rcr");
  const staticOptions = {
    etag: true,
    lastModified: true,
    setHeaders: (res, path) => {
      if (path.endsWith(".html")) {
        ...
        res.header(
          "Cache-Control",
          `max-age=31536000`
        ); /* Aggressively cache other static content */
      }
    },
  };
